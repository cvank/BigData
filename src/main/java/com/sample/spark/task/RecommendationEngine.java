...
public class RecommendationEngine {
    ...
    public static void main(String[] args) {
        ...
        // final Broadcast<Map<String, Double>> userContentRecencyBroadcast = sc.broadcast(userContentRecencyRDD.collectAsMap());
        // Map<String, Double> userContentRecencyMAP = userContentRecencyRDD.collectAsMap();
        ...
        // final Broadcast<Map<String, LogEntry>> stringLogEntryMapBroadcast = sc.broadcast(userMaxTimeLogEntryJavaPairRDD.collectAsMap());
        Map<String, LogEntry> userLogEntryMap = userMaxTimeLogEntryJavaPairRDD.collectAsMap();
        /*sparkSession.udf().register("test", new UDF2<Tuple2<String, LogEntry>, Tuple2<String, LogEntry>, Tuple2<String, LogEntry>>() {
            @Override
            public Tuple2<String, LogEntry> call(Tuple2<String, LogEntry> first , Tuple2<String, LogEntry> second) throws Exception {

                if(first._1().substring(0, first._1().indexOf("_")).equals(second._1())) {
                    first._2().setFrequency(first._2().getTime()/second._2().getTime());
                }
                return first;
            }
